...
for i, (image, label) in enumerate(train_dataset):
    ...
    image = image.permute(1, 2, 0).numpy() * 255  # 转换为numpy数组并恢复像素值范围
    ...
    with h5py.File('../data/CIFAR10/h5_attr_all/train/' + str(i) + '.hdf5', 'w') as f:
        f['y'] = label
        f['edge_index'] = adj
        f['x'] = center_
...
for i, (image, label) in enumerate(test_dataset):
    ...
    for v in center:
        y, x, Rx, Ry = v[0][0], v[0][1], v[1], v[2]
        cv2.rectangle(RGB_img, (x - Rx, y - Ry), (x + Rx, y + Ry), (0, 255, 0), 1)
    with h5py.File('../data/CIFAR10/h5_attr_all/test/' + str(i) + '.hdf5', 'w') as f:
        f['y'] = label
        f['edge_index'] = adj
        f['x'] = center_
        f['edge_attr'] = edge_attr
